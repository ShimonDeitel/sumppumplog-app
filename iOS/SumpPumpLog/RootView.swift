        ...
        NavigationStack {
            ZStack {
                ...
                List {
                    ...
                    Text(entry.dateISO).font(Theme.bodyFont)
                    Text(entry.status).font(Theme.bodyFont)
                        }
...
struct EntryEditorView: View {
    ...
    @State private var draft: CheckEntry
    var onSave: (CheckEntry) -> Void

    init(entry: CheckEntry?, onSave: @escaping (CheckEntry) -> Void) {
        _draft = State(initialValue: entry ?? CheckEntry())
        self.onSave = onSave
    }
